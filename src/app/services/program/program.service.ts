    ...
    return new Promise<IProgram>((resolve, reject) => {
      this.http
        .post(
          `http://localhost:8080/query`,
          {
            query: `update \`Программы обучения\` set \`Название программы\` = '${program['Название программы']}', \`Стоимость за месяц\` = '${program['Стоимость за месяц']}', \`Табельный номер преподавателя\` = '${program['Табельный номер преподавателя']}', \`Возрастная категория детей\` = '${program['Возрастная категория детей']}' where \`Идентификатор программы\` = '${program['Идентификатор программы']}'`,
            token: localStorage.getItem('token'),
          },
          {
            responseType: 'json',
