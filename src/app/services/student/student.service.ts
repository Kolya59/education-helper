    ...
    return new Promise<IStudent>((resolve, reject) => {
      this.http
        .post(
          `http://localhost:8080/query`,
          {
            query: `update \`Обучающиеся\` set \`Фамилия ребенка\` = '${student['Фамилия ребенка']}', \`Имя ребенка\` = '${student['Имя ребенка']}', \`Дата рождения\` = '${student['Дата рождения']}', \`Имя отчество родителя\` = '${student['Имя отчество родителя']}', \`Контактный телефон\` = '${student['Контактный телефон']}' where \`Код ребенка\` = '${student['Код ребенка']}'`,
            token: localStorage.getItem('token'),
          },
          {
            responseType: 'json',
